"""
predict02.py  -  Mask2Former instance segmentation inference + morphological post-processing

Strategy
--------
全画像を CROP_SIZE × CROP_SIZE (= 1024 × 1024) にリサイズして 1 回推論し、
検出されたインスタンスマスクを元解像度に復元してオーバーレイ画像を保存する。
predict01.py との差分: 各インスタンスマスクに対して
  Opening (erosion → dilation) → Closing (dilation → erosion)
のモルフォロジー変換を適用する後処理を追加。
...
Usage:
    python predict02.py <input_dir> [--weights <path>] [--output <dir>]
...
if IS_KAGGLE:
    ...
    DEFAULT_OUTPUT_DIR = Path("/kaggle/working/predict02")
else:
    DEFAULT_WEIGHTS    = Path("/content/drive/MyDrive/mask2former_exp01/best_model.pth")
    DEFAULT_OUTPUT_DIR = Path("/content/drive/MyDrive/mask2former_predict02")
...
def predict_instances(
    ...
    Flow:
        ...
        2. class_queries_logits → softmax → no-object 以外の最大スコ��を confidence に
