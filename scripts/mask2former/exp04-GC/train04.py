...
if IS_KAGGLE:
    ...
    OUTPUT_DIR = Path("/kaggle/working/exp04")
else:
    DATA_ROOT  = Path(os.environ.get("KAGGLE_DATA_ROOT")) / "cubicasa_mask2former"
    OUTPUT_DIR = Path("/content/drive/MyDrive/mask2former_exp04")
...
# ── Auxiliary losses ──────────────────────────────────────────────────────────
# Boundary-weighted Dice: 壁境界付近を重点的に学習させる (弱め)
AUX_BOUNDARY_WEIGHT = 0.05
BOUNDARY_EDGE_SCALE = 3.0   # 境界ピクセルへの重み倍率

# Instance overlap penalty: 部屋同士の重なりを排除する (強め)
AUX_OVERLAP_WEIGHT  = 0.20
OVERLAP_TOPK        = 20    # 信頼度上位何クエリ間の重なりを評価するか
...
# ─────────────────────────────────────────────────────────────────────────────
# Custom augmentation: RandomDashedLines
# ─────────────────────────────────────────────────────────────────────────────
class RandomDashedLines(ImageOnlyTransform):
    """水平または垂直の破線をランダムに描き込む。

    建築図面に現れる寸法線・中心線（点線・鎖線）をシミュレートし、
    モデルが細い製図線を壁と誤認しないよう学習させる。
    マスクには影響しない（ImageOnlyTransform）。

    Args:
        num_lines : 描き込む線の本数範囲 (min, max)
        color     : 線の RGB 色。薄いグレーを推奨
        thickness : 線の太さ（px）
        dash_len  : ダッシュ（実線部分）の長さ（px）
        gap_len   : ギャップ（空白部分）の長さ（px）
    """

    def __init__(
        self,
        num_lines: tuple[int, int] = (1, 4),
...
def get_transforms(split: str) -> A.Compose:
    ...
    letterbox = [
        ...
        A.PadIfNeeded(
            ...
            fill=0,
            fill_mask=0,         # background index for the index mask
        ),
    ]
...
def _boundary_weight_map(
    ...
    dilation: int = 3,
    edge_scale: float = BOUNDARY_EDGE_SCALE,
) -> torch.Tensor:
    """
    GT マスク群 (N, H, W) から境界重みマップ (H, W) を生成する。

    全 GT マスクを合成した「部屋領域」に dilation/erosion を適用し、
    境界付近のピクセルを edge_scale 倍に重みづけする。
    """
    merged = gt_masks.max(dim=0).values.unsqueeze(0).unsqueeze(0)  # (1,1,H,W)

    k      = dilation * 2 + 1
    kernel = torch.ones(1, 1, k, k, device=gt_masks.device)

    dilated  = F.conv2d(merged, kernel,               padding=dilation).clamp(0, 1)
    eroded   = F.conv2d(merged, kernel / (k * k),     padding=dilation).clamp(0, 1)
    boundary = (dilated - eroded).clamp(0, 1).squeeze()  # (H, W)

    return 1.0 + (edge_scale - 1.0) * boundary
...
def _dice(
    ...
    """Boundary-weighted Dice loss (スカラー). pred, target, weight: (H, W)"""
    wp    = weight * pred
    wt    = weight * target
    inter = (wp * wt).sum()
    denom = (wp * pred).sum() + (wt * target).sum()
    return 1.0 - (2.0 * inter + eps) / (denom + eps)


def boundary_aux_loss(
    pred_logits:   torch.Tensor,        # (B, Q, Hp, Wp)
    class_logits:  torch.Tensor,        # (B, Q, num_labels+1)
    mask_labels:   list[torch.Tensor],  # list of (Ni, H, W)
) -> torch.Tensor:
    """
    各画像につき:
      1. GT マスクを予測解像度 (Hp, Wp) にリサイズ
      2. IoU でクエリと GT をグリーディマッチング
      3. マッチしたペアに Boundary-weighted Dice を計算して平均

    インスタンス数が 0 の画像はスキップする。
    """
    device = pred_logits.device
    B, Q, Hp, Wp = pred_logits.shape
    total = torch.tensor(0.0, device=device)
    count = 0

    for b in range(B):
        gt = mask_labels[b]          # (Ni, H, W)
        Ni = gt.shape[0]
        if Ni == 0:
            continue
        ...
        gt_small = F.interpolate(
            gt.unsqueeze(0).float(), size=(Hp, Wp),
            mode="bilinear", align_corners=False,
        ).squeeze(0)                 # (Ni, Hp, Wp)
        gt_small = (gt_small > 0.5).float()

        pred_sig   = pred_logits[b].sigmoid()          # (Q, Hp, Wp)
        weight_map = _boundary_weight_map(gt_small)    # (Hp, Wp)

        for i in range(Ni):
            gt_i  = gt_small[i]                                           # (Hp, Wp)
            inter = (pred_sig * gt_i.unsqueeze(0)).sum(dim=(-2, -1))      # (Q,)
            union = pred_sig.sum(dim=(-2, -1)) + gt_i.sum() - inter       # (Q,)
            best_q = (inter / (union + 1e-6)).argmax()

            total = total + _dice(pred_sig[best_q], gt_i, weight_map)
            count += 1
...
def overlap_aux_loss(
    pred_logits:  torch.Tensor,  # (B, Q, Hp, Wp)
    class_logits: torch.Tensor,  # (B, Q, num_labels+1)
    topk: int = OVERLAP_TOPK,
) -> torch.Tensor:
    """
    各画像につき信頼度上位 topk クエリのマスクペア間オーバーラップを penalize する。

    loss = mean_{i<j} mean_pixels(pred_i * pred_j)

    部屋は物理的に重なれないという建築制約をソフトに課す。
    """
    device = pred_logits.device
    ...
    for b in range(B):
        # 背景クラス (最終インデックス) を除いた最大スコアで信頼度を計算
        scores = class_logits[b].softmax(-1)[:, :-1].max(dim=-1).values  # (Q,)
        k      = min(topk, scores.shape[0])
        top_idx = scores.topk(k).indices                                  # (k,)

        pred_top = pred_logits[b][top_idx].sigmoid()                      # (k, Hp, Wp)
        if k < 2:
            continue

        # 上三角ペアのオーバーラップ平均
        overlap = (pred_top.unsqueeze(1) * pred_top.unsqueeze(0)).mean(dim=(-2, -1))  # (k,k)
        upper   = torch.triu(torch.ones(k, k, device=device, dtype=torch.bool), diagonal=1)
        total   = total + overlap[upper].mean()
    ...
    for batch in pbar:
        ...
        if is_train:
            ...
            with accelerator.accumulate(model):
                ...
                # ── 補助損失 ──────────────────────────────────────────────────
                aux_b = boundary_aux_loss(
                    outputs.masks_queries_logits,
                    outputs.class_queries_logits,
                    mask_labels,
                )
                aux_o = overlap_aux_loss(
                    outputs.masks_queries_logits,
                    outputs.class_queries_logits,
                )
                loss = loss + AUX_BOUNDARY_WEIGHT * aux_b + AUX_OVERLAP_WEIGHT * aux_o
...
# GCP self-kill
# ─────────────────────────────────────────────────────────────────────────────
def gcp_shutdown() -> None:
    """GCPメタデータAPIでインスタンス情報を取得し、自分自身をシャットダウンする。"""
    import subprocess
    ...
    def fetch(path: str) -> str:
        req = urllib.request.Request(f"{meta_base}/{path}", headers=headers)
        return urllib.request.urlopen(req, timeout=5).read().decode()

    try:
        zone    = fetch("instance/zone").split("/")[-1]   # projects/PROJECT/zones/ZONE → ZONE
        name    = fetch("instance/name")
        project = fetch("project/project-id")
        print(f"[self-kill] instance={name}  zone={zone}  project={project}")
        print("[self-kill] Stopping instance …")
        subprocess.run(
            [
        ...
        print(f"[self-kill] WARNING: could not stop instance: {e}")
