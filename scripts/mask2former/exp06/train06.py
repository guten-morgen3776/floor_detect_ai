"""
Mask2Former instance segmentation fine-tuning on augmented floor plan dataset.
exp06: Fine-tuning a CubiCasa5k-pretrained model on mask2former_augmented_data_50.

exp03 からの変更点
------------------
* データを cubicasa_mask2former → mask2former_augmented_data_50 に変更。
  CubiCasa5k で学習済みのモデル (exp03-GC / exp04-GC の best_model.pth) を
  ファインチューニングする目的のデータセット。

* モデルのロード方法を変更。
  HuggingFace Hub から事前学習済み COCO 重みをロードするのではなく、
  CubiCasa5k で学習済みの .pth ファイル (--pretrained_weights) から重みをロード。
  アーキテクチャ定義のみ HuggingFace config を参照し、
  モデル重みは local .pth から strict ロードする。
  (exp03/04-GC は既に num_labels=1 で学習済みのため ignore_mismatched_sizes 不要)

* ファインチューニング向けに LR / スケジューラを調整。
  - Base LR : 1e-4 → 3e-5 (収束済みモデルを壊さない)
  - Backbone LR scale : 0.2 → 0.1 (バックボーンをより保守的に更新)
  - Warmup epochs : 2 → 1 (モデルは既に収束済みのため短縮)
  - Default epochs : 50 → 30 (少ないデータで過学習しにくくする)
  - Early stop patience : 10 → 7 (ファインチューニングは収束が早い)

exp01 からの変更点 (exp03 より継承)
------------------
...
Key design choices (inherited from exp03)
------------------
...
Usage (single GPU):
  python train06.py --pretrained_weights /path/to/best_model.pth --batch_size 2 --epochs 30 --fp16

Usage (multi-GPU, e.g. T4×2 on Kaggle):
  accelerate launch train06.py --pretrained_weights /path/to/best_model.pth --batch_size 2 --epochs 30 --fp16
"""
...
# CONFIG
# ─────────────────────────────────────────────────────────────────────────────
# アーキテクチャ定義の参照のみに使用 (重みは pretrained_weights からロードする)
DEFAULT_MODEL_ID = "facebook/mask2former-swin-small-coco-instance"

if IS_KAGGLE:
    DATA_ROOT          = Path("/kaggle/input/mask2former-augmented-data-50")
    DEFAULT_PRETRAINED = "/kaggle/input/mask2former-exp04-output/best_model.pth"
    OUTPUT_DIR         = Path("/kaggle/working/exp06")
else:
    DATA_ROOT          = Path("/content/drive/MyDrive/mask2former_augmented_data_50")
    DEFAULT_PRETRAINED = "/content/drive/MyDrive/mask2former_exp04/best_model.pth"
    OUTPUT_DIR         = Path("/content/drive/MyDrive/mask2former_exp06")
...
# ── Differential LR ──────────────────────────────────────────────────────────
# ファインチューニングではバックボーンをより保守的に更新する。
# CubiCasa5k で学習済みの特徴量を過度に変えないよう 0.1 倍に抑制。
BACKBONE_LR_SCALE = 0.1     # backbone LR = lr * BACKBONE_LR_SCALE

# ── Warmup ───────────────────────────────────────────────────────────────────
# 学習済みモデルのファインチューニングなのでウォームアップは 1 エポックで十分。
WARMUP_EPOCHS       = 1     # warmup のエポック数
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Mask2Former room instance segmentation (exp06)")
    p.add_argument("--model_id",            type=str,   default=DEFAULT_MODEL_ID,
                   help="HuggingFace model ID used to reconstruct the architecture config")
    p.add_argument("--pretrained_weights",  type=str,   default=DEFAULT_PRETRAINED,
                   help="Path to .pth of a CubiCasa5k-pretrained model (exp03-GC / exp04-GC best_model.pth)")
    ...
    p.add_argument("--epochs",              type=int,   default=30,
                   help="Max training epochs (shorter than scratch training since model is pre-converged)")
    p.add_argument("--lr",                  type=float, default=3e-5,
                   help="Base learning rate (head / decoder) — lower than scratch for fine-tuning")
    p.add_argument("--weight_decay",        type=float, default=1e-4)
    p.add_argument("--num_workers",         type=int,   default=4)
    p.add_argument("--early_stop",          type=int,   default=7,
                   help="Patience for early stopping (fine-tuning converges faster than scratch)")
    p.add_argument("--fp16",                action="store_true",
                   help="Use mixed-precision training (fp16) via accelerate")
    # ── exp03 additions (inherited) ──────────────────────────────────────────
...
def build_model(
    ...
    """CubiCasa5k-pretrained Mask2Former を .pth ファイルからロードする。

    exp03/04-GC はすでに num_labels=1 (room) で学習済みのため、
    アーキテクチャ定義のみ HuggingFace config を参照し、
    重みは local .pth から strict ロードする。
...
def save_curves(train_losses: list[float], val_losses: list[float]) -> None:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Mask2Former Loss Curve (exp06)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def main() -> None:
    ...
    if accelerator.is_main_process:
        ...
        print(f"Model (arch)        : {args.model_id}")
    ...
    model     = build_model(args.model_id, args.pretrained_weights)
