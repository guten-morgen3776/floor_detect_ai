"""
predict03.py  -  Mask2Former instance segmentation inference + morphological post-processing
...
predict02.py との差分:
  - 前処理を単純な Resize(1024, 1024) から Letterbox リサイズ
    (LongestMaxSize(1024) → PadIfNeeded(1024×1024)) に変更。
    train03.py の学習時前処理と一致させることで推論精度を向上させる。
  - マスクの復元も Letterbox 逆変換 (1024×1024 → パディング除去 → 元解像度) に対応。
...
Usage:
    python predict03.py <input_dir> [--weights <path>] [--output <dir>]
...
if IS_KAGGLE:
    DEFAULT_WEIGHTS    = Path("/kaggle/working/exp03/best_model.pth")
    DEFAULT_OUTPUT_DIR = Path("/kaggle/working/predict03")
else:
    DEFAULT_WEIGHTS    = Path("/content/drive/MyDrive/mask2former_exp03/best_model.pth")
    DEFAULT_OUTPUT_DIR = Path("/content/drive/MyDrive/mask2former_predict03")
