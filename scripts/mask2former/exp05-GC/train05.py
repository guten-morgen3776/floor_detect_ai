...
exp05: Swin-Base backbone (exp03 からの変更点).

exp03 からの変更点
------------------
* Backbone を Swin-Small → Swin-Base に変更。
  DEFAULT_MODEL_ID = "facebook/mask2former-swin-base-coco-instance"
  より大きなバックボーンにより特徴抽出能力を向上させる。
...
Usage (single GPU):
  python train05.py --batch_size 2 --epochs 50 --fp16

Usage (multi-GPU, e.g. T4×2 on Kaggle):
  accelerate launch train05.py --batch_size 2 --epochs 50 --fp16
"""
...
if IS_KAGGLE:
    ...
    OUTPUT_DIR = Path("/kaggle/working/exp05")
else:
    DATA_ROOT  = Path(os.environ.get("KAGGLE_DATA_ROOT")) / "cubicasa_mask2former"
    OUTPUT_DIR = Path("/content/drive/MyDrive/mask2former_exp05")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Mask2Former room instance segmentation (exp05)")
...
def save_curves(train_losses: list[float], val_losses: list[float]) -> None:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Mask2Former Loss Curve (exp05)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
