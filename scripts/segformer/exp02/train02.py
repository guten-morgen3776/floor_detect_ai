...
Diff from train01.py
--------------------
* Loss: HuggingFace built-in CE  →  Focal Loss + Dice Loss (hybrid)
  - All other code (model, dataloader, optimizer, scheduler, metrics) is unchanged.
"""
...
if IS_KAGGLE:
    ...
    OUTPUT_DIR = Path("/kaggle/working/exp02")
else:
    DATA_ROOT  = Path(os.environ.get("KAGGLE_DATA_ROOT")) / "cubicasa_segment"
    OUTPUT_DIR = Path("/content/drive/MyDrive/exp02")
...
def run_epoch(model, loader, optimizer, criterion, is_train: bool):
    ...
    with torch.set_grad_enabled(is_train):
        for imgs, masks in pbar:
            ...
            # ── changed: call without labels; compute loss manually ──────────
...
def main():
    ...
    # ── changed: instantiate hybrid loss ────────────────────────────────────
